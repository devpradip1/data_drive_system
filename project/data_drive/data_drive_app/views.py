...
# Model imports
from data_drive_app.models import Folder,File
...
def index(request):
    if request.user.is_authenticated:
        ...
        return render(request,'data_drive_app/index.html',context)
    else:
...
def folder(request,folderid):
    if request.user.is_authenticated:
        ...
        context = {'folderid':folderid,'files':files,'folder':folder}
        if request.method == 'POST':
            ...
            fileadd.save
        return render(request,'data_drive_app/folder.html',context)
    else:
        return redirect('signup')
# Add Folder View
        ...
        if request.method == 'POST':
            ...
            if username and password and email and cpassword and firstname and lname:
                if password == cpassword:
                    ...
                    redirect("signup")
        return render(request,'data_drive_app/signup.html')
        ...
        if request.method == 'POST':
            ...
            if username and password:
                ...
                if user is not None:
                    login(request,user)
                    return redirect('index')
        return render(request,'data_drive_app/login.html')
...
def delete_object(request, folderid):
    object = Folder.objects.get(id=folderid)
    object.delete()
    return redirect('index')
